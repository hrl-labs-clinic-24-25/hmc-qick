...
module pvp_fsm_gen 
	...
	// a set of 4 bits that indicates which dacs are in a group (group0 = 1010 means dac3 and dac1 are in group0)
	logic [3:0] group0, group1, group2, group3;

	// Group Variables (i.e. if the current group is enabled or not. Used to change/update the current DAC levels)
	logic       group_0_en, group_1_en, group_2_en, group_3_en;

	// the number of DACs in each group
	logic [2:0] number_group_0, number_group_1, number_group_2, number_group_3;
	...
	assign top_g0 = (top_0 | ~group0[3]) & (top_1 | ~group0[2]) & (top_2 | ~group0[1]) & (top_3 | ~group0[0]);
	assign top_g1 = (top_0 | ~group1[3]) & (top_1 | ~group1[2]) & (top_2 | ~group1[1]) & (top_3 | ~group1[0]);
	assign top_g2 = (top_0 | ~group2[3]) & (top_1 | ~group2[2]) & (top_2 | ~group2[1]) & (top_3 | ~group2[0]);
	assign top_g3 = (top_0 | ~group3[3]) & (top_1 | ~group3[2]) & (top_2 | ~group3[1]) & (top_3 | ~group3[0]);
	...
	// ((curr_state==S_STALL) | (dwell_counter==0)) ? past_select_mux :  (curr_state == CONFIG_STATE) ? CONFIG_REG[28:24] : (curr_state==S_SEND_0) ? DEMUX_0_REG[4:0]     : (curr_state==S_SEND_1) ? DEMUX_1_REG[4:0] : (curr_state==S_SEND_2) ? DEMUX_2_REG[4:0] : (curr_state==S_SEND_3) ? DEMUX_3_REG[4:0] : 4'b1111; // this is hard to debug if we're always seeing first DAC and none of second (lol)
