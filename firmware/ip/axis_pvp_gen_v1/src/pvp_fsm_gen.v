	...
	if (~rstn) begin
		...
		case (state)
			WAIT:
				if (tready)
					state <= LOAD_DAC2_1;
			LOAD_DAC2_1: state <= LOAD_DAC2_2;
			LOAD_DAC2_2: state <= LOAD_DAC2_3;
			LOAD_DAC2_3: state <= LOAD_DAC1_1;
			LOAD_DAC1_1: state <= LOAD_DAC1_2;
			LOAD_DAC1_2: state <= LOAD_DAC1_3;

			if (state == LOAD_DAC1_3 && reg_loaded)
				to_spi_en <= 1;
		endcase
...
assign mux_o = ? !(state == WAIT_ST)  1 : 0 // assign to 0

assign mux_one = ? (state == LOAD_DAC1_1 || LOAD_DAC1_2 || LOAD_DAC1_3) mux1[0] && mux1[1] : ? (state == LOAD_DAC2_1 || LOAD_DAC2_2 || LOAD_DAC2_3) mux2[0] && mux2[1];
assign mux_two = ? (state == LOAD_DAC1_1 || LOAD_DAC1_2 || LOAD_DAC1_3 mux2[0]) ^^ mux2[1] : ? (state == LOAD_DAC2_1 || LOAD_DAC2_2 || LOAD_DAC2_3) mux2[0] ^^ mux2[1];;
assign mux_three = ? (state == LOAD_DAC1_1 || LOAD_DAC1_2 || LOAD_DAC1_3) mux3[0] || mux3[1] : ? (state == LOAD_DAC2_1 || LOAD_DAC2_2 || LOAD_DAC2_3) mux2[0] || mux2[1];

assign to_spi = ? (to_spi_en && (state == LOAD_DAC1_1 || LOAD_DAC1_2 || LOAD_DAC1_3)) iter_dout : fifo_dout;

assign reg_loaded = ? (iter_full && fifo_full) 1 : 0;
...
// wire from FSM to fifo_2 (normal fifo)
// wire from FSM to fifo_1 (iterating fifo)

// wire containing "fifo_2_loaded" and "fifo_1_loaded" signal


// Outputs.
assign mux_1			= mux_o[0] && mux_o[1];
assign mux_2			= mux_o[0] ^ mux_o[1];
assign mux_3			= mux_o[0] || mux_o[1];
assign out_spi 			= en_la_r;
