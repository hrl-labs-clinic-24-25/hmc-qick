					...
					case (MODE_REG)
						2'b10: begin //"Spiral" increment mode							
							if (count == PVP_WIDTH_REG)begin //if index is half of DEPTH
								...
								else if ((dwell_counter == DWELL_CYCLES_REG) & top_0)  // if we've' gone full through list flip direction
								begin 
									dwell_counter <= 0; 
									direction_0 <= 0;
							...
							begin 
								...
								if ((dwell_counter == DWELL_CYCLES_REG) & !base_0)      // inside DAC 1 loop
								begin 
									dwell_counter <= 0; 
									next_state <= S_SEND_0; 
								end
								else if ((dwell_counter == DWELL_CYCLES_REG) & base_0)  // if we've' gone full through list, go to next DAC
								begin 
									dwell_counter <= 0; 
									direction_0 <= 1;
									if (NUM_DACS_REG < 2) next_state <= WAIT;
					...
					case (MODE_REG)
						2'b10: begin //"Spiral" increment mode
							if (direction_1)begin 
								...
								else if ((dwell_counter == DWELL_CYCLES_REG) & top_1)  // if we've' gone full through list flip direction
								begin 
									dwell_counter <= 0; 
									direction_1 <= 0;
									index <= index + 1;
							...
							begin 
								// cycle until dwell counter has been spent. If we're not at the base of the stack, return to stall
								if ((dwell_counter == DWELL_CYCLES_REG) & !base_1)      // inside DAC 1 loop
								begin 
									dwell_counter <= 0; 
									next_state <= S_SEND_1; 
								end
								else if ((dwell_counter == DWELL_CYCLES_REG) & base_1)  // if we've' gone full through list, go to next DAC
								begin 
									dwell_counter <= 0; 
									direction_1 <= 1;
									count <= count + 1;
					...
					case (MODE_REG)
						...
						2'b01: begin // "Top-Bottom" increment mode 
							if (direction_2)begin 
								...
								else if ((dwell_counter == DWELL_CYCLES_REG) & top_2)  // if we've' gone full through list flip direction
								begin 
									dwell_counter <= 0; 
									direction_2 <= 0;
									if (NUM_DACS_REG < 2) next_state <= WAIT;
							...
							begin 
								// cycle until dwell counter has been spent. If we're not at the base of the stack, return to stall
								if ((dwell_counter == DWELL_CYCLES_REG) & !base_2)      // inside DAC 1 loop
								begin 
									dwell_counter <= 0; 
									next_state <= S_STALL; 
								end
								else if ((dwell_counter == DWELL_CYCLES_REG) & base_2)  // if we've' gone full through list, go to next DAC
								begin 
									dwell_counter <= 0;
									direction_2 <= 1;
					...
					case (MODE_REG)
						...
						2'b01: begin // "Top-Bottom" increment mode 
							if (direction_3)begin 
								...
								else if ((dwell_counter == DWELL_CYCLES_REG) & top_3)  // if we've' gone full through list flip direction
								begin 
									dwell_counter <= 0; 
									direction_3 <= 0;
									if (NUM_DACS_REG < 2) next_state <= WAIT;
							...
							begin 
								// cycle until dwell counter has been spent. If we're not at the base of the stack, return to stall
								if ((dwell_counter == DWELL_CYCLES_REG) & !base_3)      // inside DAC 1 loop
								begin 
									dwell_counter <= 0; 
									next_state <= S_STALL; 
								end
								else if ((dwell_counter == DWELL_CYCLES_REG) & base_3)  // if we've' gone full through list, go to next DAC
								begin 
									dwell_counter <= 0;
									direction_3 <= 1;
									next_state <= WAIT;
						...
						2'b00: begin // Default mode (increment only)
							...
							else if ((dwell_counter == DWELL_CYCLES_REG) & top_3)  // if we've' gone full through list, go to next DAC
							begin 
								dwell_counter <= 0; 
								next_state <= WAIT;
								rstn_3 <= 0;
